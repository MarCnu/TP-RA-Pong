...
void ArUco::drawScene() {
	...
	*/
	...
	for (unsigned int m=0;m<m_Markers.size();m++)
	{
		...
		* BEGIN
		*/
		// Detection du premier marker pour initialiser le repere
		m_pong.detectRepere(m_Markers[m].id);

		int repereIndex = m_pong.getRepereIndex(m_Markers);

		if(repereIndex != -1) // Si le repère n'est pas visible, on met le jeux en pause, cad on ne fait rien
		{
			if(repereIndex == m) // Si c'est le repère, on déplace la balle dans le repère
			{
				m_pong.pos[0] += m_pong.speedX;
				m_pong.pos[2] += m_pong.speedZ;

				glPushMatrix();
				glTranslatef(m_pong.pos[0], 0, m_pong.pos[2]);
				glColor3f(1,0.4f,0.4f);
				...
				glPushMatrix();
				glTranslatef(0, 0, 1);
				glColor3f(0.4f,0.6f,1);
				glScalef(3, 0.3, 0.01);
				glutSolidCube(1);
				glPopMatrix();
				
				m_pong.intersectionMur();
			}
			else // Si c'est autre chose que le repère, on considère que c'est un palet
			{
				double repere_matrix[16];
				m_Markers[repereIndex].glGetModelViewMatrix(repere_matrix);
				m_pong.intersection(m_Markers[m].id, repere_matrix, modelview_matrix);
